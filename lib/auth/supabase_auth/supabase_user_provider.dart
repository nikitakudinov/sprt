...
class SupabaseRealTimeSupabaseUser extends BaseAuthUser {
  SupabaseRealTimeSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> supabaseRealTimeSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SupabaseRealTimeSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
